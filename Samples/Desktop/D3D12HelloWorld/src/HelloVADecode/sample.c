...
void VALoadPipeline(DXSample* const sample);
void VAInitDisplay(DXSample* const sample);
void VAImportRenderTargets(DXSample* const sample);
void VAPerformWorkload(DXSample* const sample);
void VADestroy(DXSample* const sample);
void VACreateSurfaces(DXSample* const sample);

/* Video Processor */

void VAEnsureProcSupport(DXSample* const sample);
void VAInitProcContext(DXSample* const sample);
...
void VAPerformBlit(
	...
	float alpha,
	uint32_t blendingFlags);

/* Video Decode */

void VAEnsureDecSupport(DXSample* const sample);
void VAInitDecContext(DXSample* const sample);
void VAPerformDecodeFrame(DXSample* const sample);
...
void VALoadPipeline(DXSample* const sample)
{
	...
	// check support for the needed features
	VAEnsureProcSupport(sample); // can pre/post-process video and blit R8G8B8A8_NV12 -> R8G8B8A8_UNORM
	VAEnsureDecSupport(sample);  // can decode H264
	// create the sample->VASurfaceNV12, where we will render into
	VACreateSurfaces(sample);
	VAImportRenderTargets(sample);
	// create contexts
	VAInitProcContext(sample);
	VAInitDecContext(sample);
}
...
void VAEnsureProcSupport(DXSample* const sample) {
	...
	VAStatus va_status = vaQueryConfigEntrypoints(sample->vaDisplay, VAProfileNone, entrypoints, &num_entrypoints);
...
void VAImportRenderTargets(DXSample* const sample) {
	...
	for (size_t i = 0; i < FrameCount; i++)
	{
		...
		// sample->device and ???
		HRESULT hr = CALL(CreateSharedHandle, sample->device,
...
/*******************************************************************************************
   Create the decode context (sample->VADecContextId) and fill the buffers (identified by 
   the ids in sample->VADecPipelineBufferId[4]) that will be used as data when rendering 
...
void VAPerformWorkload(DXSample* const sample)
{
	// Decode ("render") H264 data into a NV12 surface
	VAPerformDecodeFrame(sample);
	// Color convert NV12 into the RGBA render target
	VAPerformBlit(sample->vaDisplay,
		sample->vaColorConvCtx,
		sample->vaColorConvBuf,
		&sample->VASurfaceNV12,
		1,
		...
		1.0f,
		sample->ProcPipelineCaps.blend_flags
	);
}

/* Decode the frame into the sample->VASurfaceNV12 */
void VAPerformDecodeFrame(DXSample* const sample) {
	VAStatus va_status = vaBeginPicture(sample->vaDisplay, sample->VADecContextId, sample->VASurfaceNV12);
	vaExitIfFailed(va_status, "vaBeginPicture");

	vaRenderPicture(sample->vaDisplay, 
		sample->VADecContextId, 
		sample->VADecPipelineBufferId, 
		_countof(sample->VADecPipelineBufferId)
	);
...
void VAPerformBlit(
	...
	uint32_t blendingFlags)
{
	assert(inSurfacesCount == 1); // why ?

	VAStatus va_status = vaBeginPicture(display, context, dstSurface);
	vaExitIfFailed(va_status, "vaBeginPicture");
	...
	for (size_t i = 0; i < inSurfacesCount; i++)
	{
		...
		VABlendState blend;
		if (blendingFlags & VA_BLEND_GLOBAL_ALPHA)
		{
...
void VADestroy(DXSample* const sample) {
	...
	// DestroyDec
	va_status = vaDestroyConfig(sample->vaDisplay, sample->VADecConfigId);
	vaExitIfFailed(va_status, "vaDestroyConfig");
	...
	// DestroySurfaces
	va_status = vaDestroySurfaces(sample->vaDisplay, sample->VARenderTargets, FrameCount);
