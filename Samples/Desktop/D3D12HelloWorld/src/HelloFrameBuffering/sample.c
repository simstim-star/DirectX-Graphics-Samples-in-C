...
static void WaitForGpu(DXSample* const sample);
static void MoveToNextFrame(DXSample* const sample);
...
void Sample_Init(DXSample* const sample) {
	...
	memset(sample->fenceValues, 0, FrameCount);
...
static void LoadPipeline(DXSample* const sample)
{
	...
	/* Create frame resources on the descriptor heaps above */
	{
		...
		for (UINT nthSwapChainBuffer = 0; nthSwapChainBuffer < FrameCount; nthSwapChainBuffer++)
		{
			...
			ExitIfFailed(CALL(CreateCommandAllocator, sample->device, D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&sample->commandAllocators[nthSwapChainBuffer])));
		}
	}
	RELEASE(factory);
}
...
static void LoadAssets(DXSample* const sample)
{
	...
	{
		ExitIfFailed(CALL(CreateFence, sample->device, 
			sample->fenceValues[FrameCount], 
			D3D12_FENCE_FLAG_NONE, 
			IID_PPV_ARGS(&sample->fence))
		);
...
// Wait for pending GPU work to complete. Only used when finished loadind assets and destroying the sample.
static void WaitForGpu(DXSample* const sample)
{
	// Schedule a Signal command in the queue.
	ExitIfFailed(CALL(Signal, sample->commandQueue, sample->fence, sample->fenceValues[sample->frameIndex]));
	// Wait until the fence has been processed.
	ExitIfFailed(CALL(SetEventOnCompletion, sample->fence, sample->fenceValues[sample->frameIndex], sample->fenceEvent));
	WaitForSingleObjectEx(sample->fenceEvent, INFINITE, FALSE);
	// Increment the fence value for the current frame.
	sample->fenceValues[sample->frameIndex]++;
}

// Prepare to render the next frame, instead of waiting for the work on the 
// current to finish, as we did in HelloTriangle
static void MoveToNextFrame(DXSample* const sample)
{
	// Schedule a Signal command in the queue.
	const UINT64 currentFenceValue = sample->fenceValues[sample->frameIndex];
	ExitIfFailed(CALL(Signal, sample->commandQueue, sample->fence, currentFenceValue));
	// Go to the other buffer, that was in the back
	sample->frameIndex = CALL(GetCurrentBackBufferIndex, sample->swapChain);
	// If it is not ready to be rendered yet, wait until it is ready.
	if (CALL(GetCompletedValue, sample->fence) < sample->fenceValues[sample->frameIndex])
	{
		ExitIfFailed(CALL(SetEventOnCompletion, sample->fence, sample->fenceValues[sample->frameIndex], sample->fenceEvent));
		WaitForSingleObjectEx(sample->fenceEvent, INFINITE, FALSE);
	}
	// This fenceValue will indicate that the work on this buffer has ended
...
static void PopulateCommandList(DXSample* const sample)
{
	...
	ExitIfFailed(CALL(Reset, sample->commandAllocators[sample->frameIndex]));

	// However, when ExecuteCommandList() is called on a particular command 
	// list, that command list can then be reset at any time and must be before 
	// re-recording
	ExitIfFailed(CALL(Reset, sample->commandList, sample->commandAllocators[sample->frameIndex], sample->pipelineState));
